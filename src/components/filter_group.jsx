...
class FilterGroup extends React.Component {
	...
	getFilterItemList(){
		...
		for(var x = 0; x < size; x++ ){
			itemArr[x] = new FilterItem(this.state.labels[x],
	...
	render(){
		...
		return(
			<div id="filter_group_container">
				<a className="filter_group_title">
				{this.state.title}
				</a>
				<div id="filter_contents_container">
					{
						itemArr.map(function(listValue, index){
							return (<div className="checkBoxItem" key={index}>
												<input name={index} type="checkBox" checked={listValue.checked}
